...
public class MMLSparser{
  ...
  public String retrRawString(){
    String outString="";
    if(target_doc != null){
      Elements pTitles = target_doc.getElementsByTag("font");
      outString += pTitles.text();

      Elements pContents = target_doc.getElementsByTag("p");
      outString += pContents.text();

      Elements pInfo = target_doc.getElementsByTag("i");
      outString += pInfo.text();
      return outString;
    }else{
