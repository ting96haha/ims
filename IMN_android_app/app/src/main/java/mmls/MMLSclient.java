...
public class MMLSclient {
  ...
  public Document getLogin(){
    ...
    try{
      ...
      Log.e("PostFragment_Client",e.toString());
  ...
  */
  ...
  private Connection login(){
    ...
    try{
        Log.v("PostFragment_BREAKPOINT","Begin Login process");
      initial.execute();
        Log.v("PostFragment_BREAKPOINT","Post Execute, retr Token");
      Response initial_response = initial.response();
      //check on the presence of the cookie for laravel session
      //the cookie expires in 2hours.
      if(initial_response.hasCookie("laravel_session")){
      ...
      login.data("stud_pswrd",stud_pw);
        ...
        Log.v("PostFragment_BREAKPOINT","Login Token: "+login_token);
        Log.v("PostFragment_BREAKPOINT","Laravel Session: "+session_cookie);
      ...
      Document login_response = login.post(); //execute the POST
        Log.v("PostFragment_BREAKPOINT","Login Execute, retr auth");
      //debugging prints
      System.out.format("Laravel Session cookie: %s\nLogin token: %s\n",session_cookie,login_token);
      ...
      Log.v("PostFragment_BREAKPOINT",login_response.title());

      return auth;
    }
    catch (Exception e){
        Log.e("PostFragmentClientLogin",e.toString());
        return null;
    }
  }
